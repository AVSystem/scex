...
/** Created: 25-10-2013 Author: ghik
  */
class PStringTest extends AnyFunSuite {
  def test(
    ...
    shiftMapping: (Int, ShiftInfo)*
  )(
    reverseShiftMapping: (Int, ShiftInfo)*
  ): Unit = super.test(name) {
  ...
  test("single add test")(Modification(0, 5, Binding.Left))(0 -> ShiftInfo(0, 5, 0, Binding.Left))(
    0 -> ShiftInfo(0, 0, 5, Binding.Left)
  )

  test("single remove test")(Modification(0, -5, Binding.Left))(0 -> ShiftInfo(0, 0, 5, Binding.Left))(
    0 -> ShiftInfo(0, 5, 0, Binding.Left)
  )

  test("remove+add replace test")(Modification(0, -5, Binding.Left), Modification(0, 3, Binding.Left))(
    0 -> ShiftInfo(0, 3, 5, Binding.Left)
  )(0 -> ShiftInfo(0, 5, 3, Binding.Left))

  test("add+remove replace test")(Modification(0, 3, Binding.Left), Modification(0, -5, Binding.Left))(
    0 -> ShiftInfo(0, 3, 5, Binding.Left)
  )(0 -> ShiftInfo(0, 5, 3, Binding.Left))

  test("consecutive remove and add test")(Modification(0, -5, Binding.Left), Modification(5, 3, Binding.Left))(
    0 -> ShiftInfo(0, 0, 5, Binding.Left),
    5 -> ShiftInfo(-5, 3, 0, Binding.Left),
  )(0 -> ShiftInfo(0, 5, 3, Binding.Left))
