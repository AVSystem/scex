...
/** Scex compiler that accepts modified, XML-friendly syntax: <ul> <li>string literals can be enclosed in both single
  * and double quotes</li> <li>identifiers 'lt', 'gt', 'lte', 'gte', 'and' and 'or' are aliases of '<', '>', '<=', '>=',
  * '&&' and '||'</li> <li>interpolation arguments must always be blocks, i.e <tt>&#36;{ident}</tt>, not
  * <tt>&#36;ident</tt> <li>dollars need not be escaped using two dollars</li> </ul>
  *
  * Created: 16-08-2013 Author: ghik
  */
