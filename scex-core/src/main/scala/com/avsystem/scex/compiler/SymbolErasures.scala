...
trait SymbolErasures { this: Global =>
  ...
  def erasureClass(tpe: Type) = try typeToJavaClass(tpe.erasure)
  catch {
  ...
  private def classToJava(clazz: ClassSymbol): Class[_] = {
    ...
    if (clazz.isPrimitiveValueClass)
      ...
      if (childOfTopLevelObject)
        ownerClazz = Class.forName(ownerClazz.getName.stripSuffix("$"), true, ownerClazz.getClassLoader)
  ...
  private final object compactifier extends (String => String) {
    ...
    /** COMPACTIFY
      *
      * The maximum length of a filename on some platforms is 240 chars (docker). Therefore, compactify names that would
      * create a filename longer than that. A compactified name looks like prefix + $$$$ + md5 + $$$$ + suffix, where
      * the prefix and suffix are the first and last quarter of the name, respectively.
      *
      * So how long is too long? For a (flattened class) name, the resulting file will be called "name.class", or, if
      * it's a module class, "name$.class" (see scala/bug#8199). Therefore the maximum suffix is 7 characters, and names
      * that are over (240 - 7) characters get compactified.
      */
    final val marker = "$$$$"
    ...
    def toMD5(s: String, edge: Int): String = {
      ...
      val cs = s.toArray
      val bytes = Codec.toUTF8(java.nio.CharBuffer.wrap(cs, 0, cs.length))
      md5.update(bytes)
  ...
  def memberToJava(symbol: Symbol): Option[Member] = try
    symbol match {
