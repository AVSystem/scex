...
class ExpressionCompiler(config: ExpressionCompilerConfig) {
   ...
   */
  private def generateJavaGetterAdapter(clazz: Class[_], full: Boolean): Option[String] = {
  ...
  private def compileFullJavaGetterAdapter(clazz: Class[_]): Try[String] = {
    ...
    val codeToCompile =
      ...
      |package $pkgName
      |
      |${generateJavaGetterAdapter(clazz, full = true).get}
      """.stripMargin
  ...
  private def adapterName(clazz: Class[_]) =
    ...
    val contextGetterAdapterCode =
      if (profile.symbolValidator.referencedJavaClasses.contains(contextClass)) {
        ...
        s"""
          ...
          |val _ctxa = new $adapterPkg.$adapterClassName(_ctx)
          |import _ctxa._
          |
        """.stripMargin
      } else ""
