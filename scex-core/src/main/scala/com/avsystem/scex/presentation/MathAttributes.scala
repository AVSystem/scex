...
object MathAttributes {
  final val scalaMathAttributes: List[SymbolInfo[Attributes]] = attributes {
    on { m: scala.math.type =>
      ...
      m.expm1 _ --> Attributes(documentation = "Return the Euler's number raised to the power of the `x` argument with one subtracted from it.")

      m.floor _ --> Attributes(documentation = "Returns the largest integer value less than or equal to the `x` argument.")
      m.floorDiv(_: Int, _:Int) --> Attributes(documentation = "Returns the largest integer value less than or equal to the algebraic quotient, " +
        "where `x` is the dividend and `y` is the divisor.")
      m.floorDiv(_: Long, _:Long) --> Attributes(documentation = "Returns the largest integer value less than or equal to the algebraic quotient, " +
        "where `x` is the dividend and `y` is the divisor.")
      m.floorMod(_: Int, _:Int) --> Attributes(documentation = "Returns the largest integer value less than or equal to the algebraic quotient, " +
        "where `x` is the dividend and `y` is the divisor.")
      m.floorMod(_: Long, _:Long) --> Attributes(documentation = "Returns the largest integer value less than or equal to the algebraic quotient, " +
        "where `x` is the dividend and `y` is the divisor.")

      m.getExponent(_: Double) --> Attributes(documentation = "Returns the size of unit in the last place of the `d` argument.")
      m.getExponent(_: Float) --> Attributes(documentation = "Returns the size of unit in the last place of the `f` argument.")
      ...
      m.rint _ --> Attributes(documentation = "Returns the closest floating point value that is equal to the integer for the `x` argument.")
      ...
      m.scalb(_: Double, _: Int) --> Attributes(documentation = "Returns `d` times 2 to the power of `scaleFactor` " +
        "rounded as if performed by a single correctly rounded floating-point multiply.")
      m.scalb(_: Float, _: Int) --> Attributes(documentation = "Returns `f` times 2 to the power of `scaleFactor` " +
        "rounded as if performed by a single correctly rounded floating-point multiply.")
