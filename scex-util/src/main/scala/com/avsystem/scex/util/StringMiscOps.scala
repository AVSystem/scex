...
final class StringMiscOps(private val wrapped: String) extends AnyVal {
  @Documentation(
    "Converts this string to date assuming it is a date in the <tt>yyyy.MM.dd HH:mm:ss</tt> format " +
      "or a number of milliseconds since 1970.01.01 00:00:00."
  )
  def toDate: JDate = try CommonDateFormat.get.parse(wrapped)
  catch {
    case _: ParseException =>
      try new JDate(wrapped.toLong)
      catch {
  ...
  @Documentation(
    "Converts this string to date by parsing it according to the specified `format` " +
      "defined with Java SimpleDateFormat. " + "Example format: <tt>yyyy.MM.dd HH:mm:ss</tt>."
  )
  def toDate(format: String): JDate = try new SimpleDateFormat(format).parse(wrapped)
  catch {
    case _: ParseException =>
  ...
  @Documentation(
    "Converts this string to date by parsing it according to specified date `format` and `timezone`. " +
  ...
  @Documentation(
    "Splits this string into a list of strings using given separator. " +
  ...
  def splitByTrim(separator: String): JList[String] =
    Lists.newArrayList(Splitter.on(separator).trimResults.split(wrapped))
  ...
  def decodeHexString: String = try new String(Hex.decodeHex(wrapped.toCharArray), StandardCharsets.UTF_8)
  catch {
  ...
  def decodeHex: Bytes = try new Bytes(Hex.decodeHex(wrapped.toCharArray))
  catch {
    case e: DecoderException =>
      throw new IllegalArgumentException(e)
  }
  ...
  @Documentation(
    "Calculates MD5 digest from contents of this string encoded using UTF-8. " +
  ...
  @Documentation(
    "Calculates HMAC MD5 digest from contents of this string. " + "The output is a 32 character hexadecimal string."
  )
