...
final class StringNetworkOps(private val wrapped: String) extends AnyVal {
  ...
  @Documentation(
    "Returns true if the string is an IPv4 address which belongs to the subnet provided as the " +
  ...
  @Documentation(
    "Returns true if the string is an IPv4 address which belongs to the subnet defined by the `subnet` and 'mask' arguments." +
  ...
  @Documentation(
    "Returns 0 if this address is equal to the one provided as the argument, -1 if less than, 1 if greater than."
  )
